...
class YTDLSource(discord.PCMVolumeTransformer):
    ...
    async def create_source(cls, ctx, search: str, *, loop, download=False):
        ...
        await ctx.send(f'```ini\n[Добавил {data["title"]} в очередь воспроизведения.]\n```', delete_after=15)
        ...
        while not self.bot.is_closed():
            ...
            if not isinstance(source, YTDLSource):
                try:
                    ...
                    await self._channel.send(f'Возникла проблема с воспроизведением песни.\n'
                                             f'```css\n[{e}]\n```')
                    continue
            ...
            self.np = await self._channel.send(f'**Сейчас играет:** `{source.title}` по заказу '
                                               f'`{source.requester}`')
...
class music(commands.Cog):
    ...
    async def music(self, ctx):
        embed = discord.Embed(title="Oora! Я Ананасик музыкант!", color=0xa500ff)
        ...
        embed.add_field(name='**;play ;sing <song_name>**', value='Найти <song_name> на ютубе и включить', inline=False)
...
#        embed.set_footer(text="")
    ...
    async def __error(self, ctx, error):
        ...
        if isinstance(error, commands.NoPrivateMessage):
            ...
            await ctx.send('Ошибка подключения к голосовому каналу. '
                           'Пожалуйста, убедитесь, что Вы в доступном мне голосовом канале и просите меня зайти в таковой.')
    ...
    async def queue_info(self, ctx):
        ...
        if not vc or not vc.is_connected():
            return await ctx.send('Но я не подключен к голосовому каналу!', delete_after=20)

        player = self.get_player(ctx)
        if player.queue.empty():
            return await ctx.send('Ой-ой! В очереди нет песен!')

        # Grab up to 5 entries from the queue...
        upcoming = list(itertools.islice(player.queue._queue, 0, 5))

        fmt = '\n'.join(f'**`{_["title"]}`**' for _ in upcoming)
        embed = discord.Embed(title=f'Дальше {len(upcoming)}', description=fmt, color=0xa500ff)
        embed.set_author(name='Господин Ананасик', icon_url='https://cdn.discordapp.com/avatars/449543738486816769/536e8a791db747e20ace0d0a3df6e070.png')
        embed.set_thumbnail(url="https://cdn.discordapp.com/avatars/449543738486816769/536e8a791db747e20ace0d0a3df6e070.png")
        embed.set_footer(text="Музыку играю! Ууии!")
    ...
    async def now_playing_(self, ctx):
        ...
        if not player.current:
            return await ctx.send('Но я ничего и не проигрываю в данный момент!')
